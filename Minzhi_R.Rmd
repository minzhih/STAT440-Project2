...
#index2[-c(index2[1],index2[1+1])]

#for(i in index[sample(1:length(index),500)])
#{
#  print(c(rawdata[i-3,14],rawdata[i-2,14],rawdata[i-1,14],rawdata[i,14],rawdata[i+1,14],rawdata[i+2,14],rawdata#[i+3,14]))
#}

longestNAstrech(rawdata$V14)

u10<-unique(rawdata$V10)
data<-rawdata[rawdata[,10]%in%u10[5],]
unique(data$V10)
unique(data$V14)
```
